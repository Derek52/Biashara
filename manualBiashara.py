...
class Coin:
    def __init__(self, name, buyTarget):
        self.name = name + 'BTC'
        self.buyTarget = buyTarget
        self.sellTarget = buyTarget1 + (buyTarget1 * (profitPercentage * .01)) #sell at the profit goal, above the buy price. .01 converts the percentage to the proper decimal. E.g. 5% * .01 = .05
...
#Since this is the manual script, you also need to edit in your buytarget for each coin, I do that in BTC values, but again, should work with ETH just fine.
coins = [Coin('BTC', .00384)]

#longer list example
#coins = [Coin('BTC', .000384),
#            Coin('ETH', .00343395),
#            Coin('ZIL', .32525)]
